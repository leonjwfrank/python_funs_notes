...
if sys.version_info[0] == 3:  # py3 导入方式
    from io import StringIO
else:
    from StringIO import StringIO
try:
    from kazoo.client import KazooClient
except ImportError as iperror:
    print(f'please install package:{"kazoo"}, msg{iperror}')
    import sys
    sys.exit()
...
class RPCHandler(asyncore.dispatcher_with_send):
    ...
    def handle_rpc(self):
        while True:
            ...
            print(f'os pid get error:{os.getpid()}, request in:{in_}, params:{params}')
...
class RPCServer(asyncore.dispatcher):
    ...
    def __init__(self, host, port):
        ...
        print(f'get host {host} port {port}')
        self.host = host
        self.port = port
    ...
    def prefork(self, n):
        for i in range(n):
            pid = os.fork()
            if pid < 0:  # fork error
                raise ValueError(f'fork error {pid}')
    ...
    def register_zk(self):
        # 127.0.0.1:2181
        self.zk = KazooClient(hosts='{}:{}'.format(self.host, self.port))
        self.zk.start()
